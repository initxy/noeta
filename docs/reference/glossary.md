...
See also: [API Reference](/reference/sdk)
...
See also: [ADR: Provider adapters and multimodal](https://github.com/initxy/noeta/blob/main/docs/adr/provider-adapters-and-multimodal.md), [ADR: Provider-neutral](https://github.com/initxy/noeta/blob/main/docs/adr/provider-neutral.md)
...
Not a first-class concept in the engine. Express fixed procedures with a deterministic Policy + `spawn_subtask`. An orchestration script the model improvises lands as **one Task + a Policy that interprets that script**. Multi-node "workflow" sequencing of root tasks is something a host builds on top of the libraries, not an engine primitive.

### "Session"

**Not a concept in these libraries.** The engine knows only Tasks, and multi-turn conversation is one Task receiving user input repeatedly; a host that groups turns into a user-visible session owns that concept itself.
