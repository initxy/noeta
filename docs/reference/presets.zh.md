...
`main` 可以并行生成三个子代理；结果是子代理的返回值，记录到 EventLog 中，以便整棵树 fold 回状态。见[ADR：子任务扇出与持久唤醒](../adr/subtask-fanout-and-durable-wake.md)和[ADR：子任务并行执行](../adr/subtask-parallel-execution.md)。
...
- 另见：[ADR：工具与代理目录](../adr/tool-and-agent-catalog.md)、[ADR：库-SDK 架构](../adr/library-sdk-architecture.md)
