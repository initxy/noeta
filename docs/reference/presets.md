...
| `memory` | Cross-task memory: `memory_write` / `memory_read` tools + auto-recall at user-message seam. Only `main` enables this; its system prompt carries the memory-policy fragment (exported as `MEMORY_POLICY_PROMPT`) telling the model what to save, what not to, and the write hygiene. |
