...
| `environment_enabled` | `True` | record and render the `<workspace-environment>` block (working directory, git repo flag, platform, task-start git branch / status / date) as the first semi-stable message; `False` wires no environment pack at all — for a host that supplies its own directory and clock context |
