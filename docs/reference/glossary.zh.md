...
另见：[核心概念](../concepts.md#task)、[ADR：任务作为唯一原语](../adr/task-as-only-primitive.md)
...
另见：[核心概念](../concepts.md#wake-resume)、[ADR：子任务扇出与持久唤醒](../adr/subtask-fanout-and-durable-wake.md)
...
另见：[预设代理](presets.md)、[ADR：工具与代理目录](../adr/tool-and-agent-catalog.md)

### Options

声明式代理配置（公开接口 `noeta.sdk.Options`）。由 `compile_options` 编译为 `AgentSpec`。**表达官方代理集和自定义代理的唯一方式。**

另见：[API 参考](api/index.md)、[配置](configuration.md)
...
另见：[核心概念](../concepts.md#policy)、[ADR：Engine-Policy-数据流](../adr/engine-policy-dataflow.md)
...
另见：[工具参考](tools.md)、[ADR：工具描述规范化](../adr/tool-description-canonical.md)
...
另见：[配置](configuration.md#provider-adapters)、[ADR：Provider adapters and multimodal](../adr/provider-adapters-and-multimodal.md)、[ADR：提供者中立](../adr/provider-neutral.md)
...
另见：[核心概念](../concepts.md#eventlog)、[ADR：事件溯源真相](../adr/event-sourced-truth.md)
...
另见：[核心概念](../concepts.md#contentstore)、[ADR：存储协议 L0](../adr/storage-protocols-l0.md)
...
另见：[核心概念](../concepts.md#engine)、[ADR：Engine-Policy-数据流](../adr/engine-policy-dataflow.md)
...
另见：[核心概念](../concepts.md#how-a-step-flows)、[ADR：Worker 租约模型](../adr/worker-lease-model.md)
...
另见：[ADR：Worker 租约模型](../adr/worker-lease-model.md)、[ADR：单写者不变量](../adr/single-writer-invariant.md)
...
另见：[核心概念](../concepts.md#dispatcher)、[ADR：Worker 租约模型](../adr/worker-lease-model.md)
...
另见：[ADR：统一上下文供给](../adr/unified-context-supply.md)、[ADR：上下文压缩](../adr/context-compaction.md)
...
常驻内容（技能、内存索引）进入上下文的通用机制。两部分：**事件记录**（`ContextContentRecorded`）+ **组装渲染**（`ContentChannelRegistry` 将每种类型渲染到半稳定段）。注册 `ContentKindSpec` 是开放扩展钩子。
_避免使用：_ Provider、ContentSource、Middleware。

另见：[ADR：模型驱动的技能调用](../adr/model-driven-skill-invocation.md)
...
另见：[ADR：事件来源标记](../adr/event-origin-marker.md)

### Memory

跨任务长期内存 v1：**写** = `memory_write` 工具，**读** = `memory_read` 工具，**常驻索引** = 内容通道租户（`kind="memory"`，policy `evolving`），**自动召回** = 主机在用户消息接缝处检索。由 `Capabilities.memory` 控制。
...
另见：[核心概念](../concepts.md#guard-observer)、[ADR：Guard-Observer 钩子](../adr/guard-observer-hooks.md)
