...
<figure markdown="span">
  ![任务生命周期 —— 统一的挂起、唤醒事件与终止退出](assets/task-lifecycle.svg){ width="820" }
  <figcaption>任务生命周期：所有等待都归结为同一个 <code>Suspended</code> 状态，加上一个带类型的 wake condition；一个 wake 事件会把任务重新入队，等待下一次 lease。</figcaption>
</figure>
...
<figure markdown="span">
  ![任务执行的一次 turn —— 目标提交、lease、step loop、finish，通过 SSE 流式输出](assets/turn-sequence.svg){ width="820" }
  <figcaption>内置 agent 的一次完整 turn：submit → lease → step loop → finish。左侧的 SSE 流是产品表面；下面的步骤 1–6 是同一张图在 runtime 层的切片。</figcaption>
</figure>
