...
- The landing points are `noeta.policies.control_tools` (the `skill` control-tool schema + sole-call) and `noeta.policies.react` (translating `activate_skills` into a `StatePatchDecision`). *(**Current state:** both moved into built-ins — the `skill` control-tool schema/translate into the `skills` built-in as a `control_tool` contribution, `ReActPolicy` into the `react` built-in; `noeta.policies` is now mechanism only.)*
...
- The conditional fingerprint fold for `Capabilities.skill_invocation` lands in `noeta.agent.spec`. *(**Current state:** the fold is now into the `AgentSpec.plugins` activation tuple, still in `noeta.agent.spec`.)*
