...
- `allowed_tools` is replace-style: setting it means only those; leaving it unset yields the full built-in set of ten tools. `disallowed_tools` always subtracts. Tools authored with `@tool` mix into the same list as objects carrying their own ref.
