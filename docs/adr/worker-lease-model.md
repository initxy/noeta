...
- Poison-task backstop: each stale-lease reclamation increments a per-task `reclaim_count`; at `reclaim_max` (default 3) consecutive no-progress reclamations, `requeue_stale()` drops the task to terminal with reason `stale_reclaim_exceeded` instead of requeueing it forever. The counter resets on real progress (heartbeat, release, controlled fail-requeue, or a fresh enqueue) — taking a lease alone does not count as progress. Both dispatcher adapters implement this identically.
