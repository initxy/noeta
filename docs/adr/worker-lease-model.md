...
- A cap transitions the dispatcher ROW only: the Dispatcher holds no EventLog handle, and giving it one would tie scheduling to the record. The worker closes the resulting split itself — after `fail()`, after a sweep, and once at startup it reads the row back and writes the lease-free `TaskFailed` the cap left unwritten (`noeta.runtime.worker.reconcile_cap_terminal`), so `fold` reaches a terminal and a parent's subtask barrier fires instead of waiting on a child that already ended.
