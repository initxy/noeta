...
- **compose 时的 reminders** 是追加在尾部最末端的纯渲染器：未完成的 todos、压缩抖动时的一条阅读策略提示，以及——一旦压缩折叠过一段前缀——一条指向折叠区间的指针，`RecallHistory` 工具可以把那段原文读回来。它们只属于 View——从不写入消息流，也从不记录为事件——并在每次 compose 时从 fold 状态重新推导，因此恢复时它们能免费重现。
