...
| `NOETA_AGENT_STORAGE` | URL | *(无)* | 用于持久化 EventLog + ContentStore + Dispatcher 的存储 URL：SQLite 文件路径或 `postgresql://` DSN（需要 `noeta-runtime[postgres]`）。未设置 = 内存（无持久化）。旧写法 `NOETA_AGENT_SQLITE` 仍被接受。 |
