...
| `NOETA_AGENT_MEMORY_CONSOLIDATION` | bool | `true` | 后台记忆整理：在会话停止接缝（close / turn 边界）经防抖触发，由隐藏的整理 agent 对长期记忆做合并 / 归档 / 补写。见 `docs/adr/memory-consolidation.md`。 |
| `NOETA_AGENT_MEMORY_CONSOLIDATION_DEBOUNCE_HOURS` | float | `24.0` | 两次整理之间的最小间隔小时数（记录在 memory root 下的 `.consolidation-state.json`）。 |
| `NOETA_AGENT_OTLP_ENDPOINT` | URL | *(无)* | OTLP trace 导出：**完整的** OTLP/HTTP traces URL（如 `http://localhost:4318/v1/traces`）。任务 / 工具 / LLM 执行会作为 span 导出到任意 OTLP collector（Jaeger、OpenTelemetry Collector 等）。不设置 = 关闭导出。导出**只能通过本变量或配置键 `otlp_endpoint` 显式开启**——环境中残留的 `OTEL_EXPORTER_OTLP_ENDPOINT` 不会静默开启它。开启后，OTel 标准变量 `OTEL_EXPORTER_OTLP_HEADERS`（`k=v,k2=v2`，值按规范百分号编码）用于补充请求头。 |
...
| `memory_consolidation` | bool | `true` | 后台记忆整理（见上方环境变量）。 |
| `memory_consolidation_debounce_hours` | float | `24.0` | 整理防抖窗口。 |
