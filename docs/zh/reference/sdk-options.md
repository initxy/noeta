...
| `memory_read_only` | `False` | 只给 `memory_read` 和 `memory_search`，用在记忆库由别人来写的场合（整理任务、另一个 agent、运维）。`memory_write` 和 `memory_archive` 直接不出现在工具列表里，而不是调了再拒绝；索引和自动召回不变，`__consolidation__` 整理 agent 仍然四个都有 |
