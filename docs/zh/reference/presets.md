...
| `memory` | 跨任务记忆：`memory_write` / `memory_read` / `memory_search` / `memory_archive` 工具 + 在用户消息接缝处自动召回。只有 `main` 启用此功能；其 system prompt 携带记忆政策片段（导出为 `MEMORY_POLICY_PROMPT`），告诉模型什么值得记、什么不记以及写入卫生规则。 |
