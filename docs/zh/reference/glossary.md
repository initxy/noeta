...
另见：[ADR：Provider adapters and multimodal](https://github.com/initxy/noeta/blob/main/docs/adr/provider-adapters-and-multimodal.md)、[ADR：提供者中立](https://github.com/initxy/noeta/blob/main/docs/adr/provider-neutral.md)
...
在 Engine 中不是一等概念。用确定性 Policy + `spawn_subtask` 表达固定过程。模型即兴创作的编排脚本表现为**一个 Task + 一个解释该脚本的 Policy**。多节点的"workflow"根任务顺序编排是 host 在库之上自行构建的，不是 Engine 原语。

### "Session"

**不是这些库里的概念。** Engine 只认识 Task，多轮对话就是一个 Task 反复接收用户输入；把多个轮次归并成一个用户可见 session 的 host 自己拥有这个概念。
