...
跨任务的长期记忆：基于文件、由模型管理。改动走 `memory_write` 和 `memory_archive`（退役，绝不删除）；读取走 `memory_read` 和 `memory_search`。**常驻索引**是一个内容通道使用者，因此压缩绝不会把它冲掉，而**自动召回**是 `turn_intake` 接缝上的一个 provider：tier-1 命中会变成 `memory` 类别的常驻内容（一条记忆正文在一个 task 里只进入一次，activate-once，压缩后仍在），tier-2 命中则以指针行的形态落在一条 `origin="memory"` 的消息里；模型自己已经用 `memory_read` 读过的页，只要那次读取还在模型看得到的历史里，两级都不再注入。召回按字面 token 匹配（名字、摘要，以及 frontmatter 的 `keywords` 别名——确定性的跨语言桥梁）。tier-1 的意思是这条消息**点了这页的名**：消息里出现了页名中的两个词，页名不足两个词时要全部出现；库里超过 `max(3, 10%)` 个页名都带的词不算（项目前缀、带日期页名里的年份）。只撞上页名里一个词、摘要里两个词，或者一条 keywords 短语，算 tier-2；同一级里证据多的排前面，每个 goal 最多带 5 条。tier-1 命中的页会把自己 frontmatter 里 `related` 列的页（写成一行：`related: a, b`）以指针形态带上；只带直接列出的这些页，不再顺着它们往下找。设置 `Options.recall_model` 后，没有点到任何页名的 goal 会再经一次小模型调用（**recall judge**：读消息加索引、挑出相关记忆），指针已经占满名额时不调；选中项以指针形态注入并照常落账。`HostConfig.recall_exclude` 列出永不召回的页，`HostConfig.memory_max_bytes` 限制 `memory_write` 正文的大小；`HostConfig.memory_read_only` 只给 `memory_read` 和 `memory_search`，用在记忆库由别人来写的场合。页名可以用任何文字的字母和数字，外加 `.` `_` `-`。`memory_write` 会盖上 `created` / `updated` 日期和一条 `source_task` 账本回执。由 `plugins=("memory", …)` 激活，属于 agent 身份的一部分——在官方 agent 中只有 `main` 打开它。
