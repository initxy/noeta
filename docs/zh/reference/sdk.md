...
来自内核词汇模块 `noeta.runtime.app_preview` / `noeta.runtime.mcp` 的相关重新导出：`AppPreviewGateway`、`AppMount`、`McpServerSpec`（stdio）、`McpHttpServerSpec`、`McpAnyServerSpec`（它们的联合）、`McpError`、`McpConfigError`、`HttpPostFn`。（MCP 连接器与 app 工具的*实现*位于 `mcp` / `app` 内置插件中——`noeta.builtins.mcp.impl` / `noeta.builtins.app.impl`。）
