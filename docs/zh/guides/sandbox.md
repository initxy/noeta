...
- 容器里的前台 `Bash` 和本地一样能停下来：中断、取消、关闭会话都会在容器里把命令杀掉，调用立刻返回，结果标成"已中断"；跑过 `timeout` 的命令也会在容器里被杀掉。被停下的命令，已经输出的那部分不会带回来。
- `Read` 读容器里的大文件时按 1 MiB 一块分批取，不再整个文件一次拉回来；展示给模型的那一段和留档的内容跟本地一样。
