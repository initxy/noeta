...
//
// Navigation contract: every published section is reachable from the top nav
// (one dropdown per section) AND from a single global sidebar that is the same
// on every page. Path-scoped sidebars are deliberately not used — they hid
// whole sections from readers who had not already guessed the URL.
