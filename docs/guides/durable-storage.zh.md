...
# PostgreSQL（psycopg 驱动随 noeta-runtime 一起安装）
...
- **PostgreSQL** 把同样的三个 adapter 放到数据库服务器上——当存储需要离开本机（托管数据库、备份、运维工具）时选它。驱动（`psycopg[binary]`，自带 libpq）随 noeta-runtime 一起安装，无需额外操作。
