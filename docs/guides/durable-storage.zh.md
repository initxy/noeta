...
默认情况下，Noeta 使用内存存储运行——对话和会话列表在进程退出时消失。要在重启后持久化会话，请将后端指向一个 SQLite 文件。

## 启用 SQLite 持久化 { #enabling-sqlite-persistence }
...
将 `sqlite_path` 添加到你的 `NOETA_AGENT_CONFIG` JSON：
...
一个 SQLite 文件支持所有三个存储 adapter：

| Adapter | 存储内容 |
| --- | --- |
| `SqliteEventLog` | 每任务的 `EventEnvelope` 记录——每一步的完整历史（消息、工具调用、决策、状态变化）。 |
| `SqliteContentStore` | 大于 4 KB 事件载荷上限的内容寻址 blob：完整的 LLM 请求/响应主体、大型工具输出、上传的图片。 |
| `SqliteDispatcher` | Worker 租约状态 + 唤醒事件队列。让重启的进程回收过期租约并交付待处理的唤醒事件。 |

文件在首次写入时自动创建。使用 `:memory:` 获取内存中的 SQLite 实例——对测试有用。

## Fold 恢复如何工作 { #how-fold-recovery-works }

当后端以设置了 `sqlite_path` 启动时，它：

1. 在同一文件上打开三个 adapters。
...
- **`NOETA_AGENT_SQLITE`** 是环境变量；`sqlite_path` 是配置文件键。SDK 默认值是内存（无文件）。
- **一个文件，三个 adapters。** `open_sqlite_storage()` 一起构建所有三个，因此 event log 已经将 dispatcher 作为其 `lease_validator` 持有。
