...
> **为服务端托管 AI agent 而生——不是给 notebook 用的。** 持久化事件溯源执行、多 worker / 多主机调度、每会话沙箱容器、完整审计与 replay、provider 中立的 LLM 接线。可自托管，无厂商绑定。

Noeta 是一个**在服务端运行 AI agent 的运行时**。它在一条**持久化 event log** 之上托管、记录、调度和回放 agent 执行——和生产系统用来保证 exactly-once 投递与崩溃安全的是同一根脊梁。这一个设计选择，换来了普通进程内 agent 库给不了你的东西：

- **崩溃安全的执行。** 任务扛得住进程被杀，从中断处原样恢复——状态从日志 fold 出来，从不攥在内存里。
- **长程任务。** 任务可以挂起数小时甚至数天，去等人工、定时器或 sub-task，等条件满足时被*精确唤醒一次*。
- **完整审计与 replay。** 每次 LLM 调用、工具调用、审批都是一个记录下来的事件，所以你能审查*为什么*某一步发生，而不只是*发生了什么*。

因为它是为**服务端**而生的——多租户、长运行、跑不可信代码——它把这件事要求的东西都补齐了：

- **多 worker / 多主机** 调度，基于共享 Postgres（lease fencing、数据库时钟过期）——加 worker 就能横向扩展。
- **每会话沙箱** —— 打开一个开关，每个会话就跑在自己那个用完即弃的 Docker 容器里；所有 fs / shell / skill / web 工具调用都在容器*内部*执行，绝不落在宿主机上。
- **端到端 human-in-the-loop** —— 审批、结构化提问、定时器 wake 都能跨重启存活。
- **Provider 中立** —— Anthropic 和任意 OpenAI 兼容模型都在同一套内部协议背后；换厂商不用重写历史。

而且整套技术栈通过确定性的 `stub` provider **离线、无需 API key** 就能跑通，三十秒上手。
...
- **每会话沙箱是可选项** —— 默认关闭（需要本地 Docker daemon + AIO 镜像）。它为每个会话起一个容器，隔离「进程 + 挂载 FS」，而非完整牢笼；warm pool / pause-resume 和跨机重连是后续工作（面向分布式 / NAS 的 provider 缝已经预留好）。
