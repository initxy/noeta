...
> 面向服务端运行 AI agent 的开源可自托管运行时——持久、可审查、provider 中立，且每会话沙箱隔离。
...
因为它生来就是为了在**服务端**托管 agent、而不只是在 notebook 里跑，它把这件事需要的东西补齐了：基于共享 Postgres 的**多 worker / 多主机**调度，以及**每会话沙箱**——打开一个开关，每个会话就跑在自己那个用完即弃的 Docker 容器里，所有 fs / shell / skill / web 工具调用都在容器*内部*执行，而不是在宿主机上。
...
- **每会话隔离** —— 打开沙箱，每个会话都拿到自己全新的 Docker 容器；每次 fs / shell / skill / web 工具调用都在容器*内部*执行，绝不落在宿主机上，一个会话的文件和进程对另一个不可见。为托管你并不完全信任的 agent——以及运行你并不完全信任的代码——而生。
