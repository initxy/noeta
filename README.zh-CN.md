...
<p align="center">
  <img src="docs/assets/crash-resume.gif" alt="对运行中的 worker kill -9，第二个进程从日志 fold 回任务并完成它" width="820">
  <br>
  <em>对运行中的 worker <code>kill -9</code>，第二个进程从日志把任务 fold 回来并跑完——<a href="examples/crash_resume.py">examples/crash_resume.py</a>，完全离线。</em>
</p>
...
- **Human-in-the-loop / 定时器 wake** —— 审批、结构化提问、定时器 wake 均已端到端可用；缺的是任务开始等待人工时的带外通知（webhook / 收件箱）。
