...
|---|---|---|---|
| **定位** | 持久化、面向 server 的 agent 运行时 | 在 Google Cloud（Gemini）上构建 agent | 计算机操作：鼠标、键盘、屏幕 |
| **部署形态** | 多 worker 池；Postgres 上多 host | 客户端库，单进程 | 桌面进程 |
| **持久化** | 事件账本 —— `state = fold(log)` | 会话状态，由平台托管 | 短暂态，无持久执行模型 |
| **挂起 / 唤醒** | 一等公民：人工、定时器、subtask、外部事件 —— 精确一次 | 会话恢复 | 不适用 |
| **模型锁定** | 无 —— 接一个适配器即可更换 provider | Gemini 优先 | 任意 LLM（它是控制层） |
| **扩展** | 16 个插件 surface + 单写者规则 | 工具与 hook | 计算机操作原语 |
| **审计 / replay** | 完整事件日志，fold 可复现 | 有限 | 无 |

**当 agent 的*运行过程*本身必须是一条能 replay、能审计、能跨 worker 与 host 扩展的账本时，就该用 Noeta** —— 而不只是一个能调用的循环。更长的展开（含 LangGraph 与 Claude Agent SDK）：[对比](https://initxy.github.io/noeta/zh/reference/comparison/)。
