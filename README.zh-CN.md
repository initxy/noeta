...
<p align="center">
  <img src="docs/assets/crash-resume.gif" alt="运行中的 agent 被中途杀掉；一个全新进程从日志把它重建出来并跑完" width="820">
  <br>
  <em><strong>崩溃安全，实测演示。</strong> 运行中的 agent 在任务中途被 <code>kill -9</code>——没有清理、没有 flush。一个全新进程重新打开同一个 store，从 event log 把任务的准确状态重建出来，并跑到完成，精确一次。没有任何东西留在内存里会丢。—— <a href="examples/crash_resume.py">examples/crash_resume.py</a>，完全离线。</em>
</p>
