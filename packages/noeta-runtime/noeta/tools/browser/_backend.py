"""``BrowserBackend`` — the seam under the noeta-owned browser tool pack.
...
sandbox-browser-subsystem spec, D1). This module keeps only the narrow
:class:`BrowserBackend` Protocol (the injection point tools call through, and
the seam tests substitute). The one concrete implementation — the AIO Sandbox
``/mcp`` adapter — lives in the ``sandbox`` built-in plugin
(``noeta.builtins.sandbox.impl.browser``) since microkernel M2, resolved by
the SDK's ``SandboxExecEnvManager`` through the loader's dynamic-import
doorway.
"""
