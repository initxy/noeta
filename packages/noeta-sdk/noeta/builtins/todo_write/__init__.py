...
(``_ACTIVATION_CAPABILITY_FLAG``), so ``plugins=("todo_write",)`` still folds to
``Capabilities.todo_write=True`` — naming the plugin the same does not change
activation semantics (the capability-flag branch resolves first).
"""
