...
Hygiene:
  ...
  - Set `keywords` in every language the user works in: auto-recall matches literal tokens, so a memory keyed in one language is invisible to questions asked in another.
