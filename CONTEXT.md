...
**Channel**:
A space's persistent group-chat surface: one message stream plus **@Agent topics**. One channel = **one persistent session** (sentinel user `__channel__`, long-lived container); each @Agent mention opens a **topic**, which is one root task on that session, and follow-ups inside a topic route to that task without re-mentioning. What distinguishes a channel from several independent sessions is **channel context**: a new topic's goal is prefixed with recent main-stream messages plus an index of past topics. Agent replies are not persisted to the message table — the topic view replays per-task from the EventLog.
_Avoid_: Chat room, Thread (a topic is a task, not a message thread)

**Board**:
The space's task board: one board per space with three fixed columns (`todo` / `doing` / `done` — columns are not entities). Cards are primarily created and managed by **people**; `links_json` carries backlinks to collaboration artifacts (topics / sessions), and `position` is an in-column REAL ordering (the frontend inserts at the midpoint between neighbours to avoid renumbering a column).
_Avoid_: Kanban engine, Issue tracker, Sprint

**Template**:
A saved, space-scoped starting point for a session. Two kinds: a **single-node template** (a parameterized prompt) and a **workflow template** (an ordered multi-node definition — a workflow session owns one root task per node and advances node to node through a handoff). Validation is advisory: a save with warnings is allowed rather than blocked.
_Avoid_: Preset (that is the SDK-level agent recipe), Macro
...
**"Session"**:
An **application-layer concept only** (see the Application layer vocabulary above; the superseding decision is `docs/adr/server-platform-product.md`). Below the application layer it remains a non-concept: the engine knows only Tasks, and multi-turn conversation is simply one Task receiving user input repeatedly — each question = one **turn** (a cycle of one wake → several Steps → suspend, with the Task resting at `suspended` + `HumanResponseReceived` between turns); each delegation = one **Subtask**. (`AgentSessionRunner` — the old L3 orchestration runner that once carried this name below the app layer — is gone; its prepare-time wiring moved into `InteractionDriver.seed_start`. Only comments still reference it.) **Do not** let session ids or session event schemas appear in engine/SDK code or below-app identifiers; the product wire may key on session ids because the application owns that wire (its event vocabulary is the translated UI events, not a hand-rolled engine schema).
