"""Read-only projections over a recorded task: the detail fold and the event tail.

Both are pure read models over the durable event stream, so they have to
tolerate every stream a run can produce — delegation and MCP sessions included,
which an operator must be able to observe even when they cannot be resumed —
and they must never raise while doing it: a viewer that crashes on one odd
event blinds the operator to the whole task. ``tail_event_rows`` additionally
owns the polling contract a follower depends on: strictly ascending sequence
numbers, no duplicates, and ``after_seq`` never re-delivering.
"""
...
def _record_next_goal(db: Path, ws: Path) -> str:
    """A next-goal-suspended session recorded through the real driver over
    sqlite, so the fold reads production-shaped events."""
...
def _record_with_skill(db: Path, ws: Path) -> str:
    """Same, plus a workspace skill activated before the loop starts via the
    driver's ``activations=`` selector."""
...
def test_inspect_next_goal_summary_fidelity(tmp_path: Path) -> None:
    ...
    # The canonical agent name is recorded, never the alias it was reached by.
...
def test_inspect_approval_call_id_visible(tmp_path: Path) -> None:
    ...
    # These are the fields an approval flow needs to act on the suspend.
    detail = _detail(db, task_id)
    assert detail is not None
...
def test_inspect_mcp_session_observable(tmp_path: Path) -> None:
    """An MCP session folds into an ordinary detail; observing is never gated."""
    db = tmp_path / "s.db"
    log, cs, disp = _seed(db)
    _emit_created_started(log, "t1")
    body = json.dumps(
...
def test_inspect_delegation_session_observable(tmp_path: Path) -> None:
    """A delegation session likewise folds into an ordinary detail."""
    db = tmp_path / "s.db"
    log, cs, disp = _seed(db)
    _emit_created_started(log, "t1")
...
# ---------------------------------------------------------------------------
# plan / todo surface in the detail fold
# ---------------------------------------------------------------------------
...
def test_inspect_plan_non_native_value_plainified(tmp_path: Path) -> None:
    """A todo carrying a non-JSON-native value is plainified, so a downstream
