...
A runtime wired with the deterministic :class:`StubProvider` runs a single
task to terminal status with no API key, no network, and a deterministic
two-turn LLM response sequence.

The behaviour under test — the stub double driving the ReAct loop's
tool-call + finish branches to a terminal task — is reached through the
assembly seam :func:`noeta.testing.profile.build_runtime` plus
:class:`noeta.testing.stub_provider.StubProvider`. The assertions (terminal
status, ``task-`` id prefix, recorded events, no-key path) pin that path.
"""
