"""``HookObserver._call_names`` holds only in-flight tool calls.

``ToolCallStarted`` records ``call_id -> tool_name`` so the matching
``ToolResultRecorded`` can name the tool a rule matched against; that result
also evicts the entry. Skipping the eviction would retain one entry per tool
call for the whole lifetime of the session and all its subtasks — a steady,
unbounded leak in a long-running host.
"""
...
def test_names_lock_present_and_guards_dict() -> None:
    # _call_names is mutated from the emit path of a log written by many
    # threads, so it carries its own lock like the sibling Audit/Metrics
    # observers.
    obs, _ = _observer()
    try:
