"""Tool-call approval through the SDK host.

These tests drive the ``SdkHost`` + ``InteractionDriver`` assembly directly:
the code stub provider proposes a ``glob`` call, the gated tool is supplied
via the host ``require_approval_tools`` knob, and the approve/deny decision is
applied through ``driver.approve`` / ``driver.deny``.
...
def _drive_approval(driver, host, out, *, approved: bool, reason: Optional[str]):
    """Resolve the pending gated tool call:
