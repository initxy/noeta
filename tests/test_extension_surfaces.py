"""Four extension surfaces: ``tool_result_transform``, ``policy``,
``prompt_fragment`` ordering, and ``sandbox_provider``.

Each surface is exercised for collision, ordering, and activation scoping,
plus its surface-specific guarantee: the transformed result is the ONLY thing
recorded; policy is single-valued per agent; sandbox_provider is host-plane and
never follows activation.
"""
...
# ===========================================================================
# tool_result_transform: a ToolRuntime pipeline stage, applied BEFORE
...
def test_transform_redaction_leaves_no_secret_in_ledger_or_content_store() -> None:
    """The transformed ToolResult is recorded; the secret reaches
...
# ===========================================================================
# policy: single-valued per agent.
# ===========================================================================


class _FakePolicyFactory:
    """A ``(llm) -> Policy`` factory carrying its identity ``.ref``."""
...
# ===========================================================================
# prompt_fragment: appended after the prompt, sorted (plugin, name).
# ===========================================================================
...
# ===========================================================================
# sandbox_provider: host-plane; never follows per-agent activation.
# ===========================================================================
...
def test_builtin_sandbox_plugin_declares_the_aio_adapters() -> None:
    """The AIO adapters ride the built-in ``sandbox`` plugin."""
    ...
    # execution) — in the plugin's own impl package.
...
# --- resolve out of the PluginSet, wire onto the host, allocate + reattach
#     across resume, and prove the key is never durable. ---
...
def test_sandbox_provider_end_to_end_from_plugin_surface_to_reattach(
    ...
    """A SINGLE flow through the plugin surface: a host resolves
