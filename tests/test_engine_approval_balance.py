"""A mixed-batch approval suspend keeps tool_use / tool_result balanced.

Every ``tool_use`` block the model emitted in one assistant turn needs a
matching ``tool_result`` before the next compose → decide, or the resumed
provider request carries a dangling function call that Anthropic / OpenAI
reject with a fatal 400. A parallel ``ToolCallsDecision`` that suspends mid-way
for human approval is where that is hardest: results of the calls that already
ran must be durably flushed before the suspend, and the trailing calls that were
never invoked need a synthesized "skipped" result. Only the approval-requiring
call itself is left for the resume path to balance.
"""
...
# ---------------------------------------------------------------------------
# An auto-allowed call BEFORE the approval-requiring one.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# An auto-allowed call AFTER the approval-requiring one.
# ---------------------------------------------------------------------------
