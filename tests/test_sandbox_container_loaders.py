"""Skill index / script / workspace loaders read through the container.

Under a sandbox every one of these reads or executes THROUGH the session's
ExecEnv rather than the host filesystem, and every path they carry is a
CONTAINER path. Both halves matter: reading a container path against the host FS
finds nothing (or, worse, the wrong file), and a skill whose rendered base
directory names a host path sends the model somewhere it cannot reach.

An in-memory container fake stands in for the backend, so no daemon runs and no
socket opens.
"""
...
# --------------------------------------------------------------------------- #
# skill indexer through the container
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# run_skill_script through the container
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# workspace loaders through the container
# --------------------------------------------------------------------------- #
