"""ProcessRegistry holds two invariants under concurrency.

The per-session concurrency cap must hold ONE lock across the count-check AND
the table insert, or two racing spawns at cap-1 could both pass the ceiling and
overrun ``max_jobs_per_root_task``. ``poll`` must snapshot ``status`` +
``exit_code`` together under the per-job lock, or a reap landing mid-poll yields
an inconsistent pair (a 'running' job that already carries an ``exit_code``).
"""
...
def test_concurrent_spawns_never_overrun_cap(tmp_path: Path) -> None:
    """N threads racing ``spawn`` on ONE session at cap-1 must never leave more
    than ``cap`` jobs RUNNING: the count-check and the insert have to share one
    lock region, or racers can both pass the ceiling."""
...
def test_poll_status_exit_code_pair_is_consistent(tmp_path: Path) -> None:
    """A terminal poll never reports a 'running' status alongside an
