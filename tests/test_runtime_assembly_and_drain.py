"""Runtime assembly and the drain loop that runs leased tasks.

``run_leased_task`` is a three-state machine — drained, woken, skipped — and the
skipped branch is the one that matters for durability: a lease handed out on a
suspended task whose wake was lost between leases must be released with
``wake_on`` intact, or the task is stranded with no way back. The wake path owes
the same fidelity in the other direction: the delivered wake event has to land
verbatim in the durable ``TaskWoken`` envelope.
"""
...
def _drain(bundle: Any) -> dict[str, int]:
    """Drive the dispatcher's ready queue empty, tallying each
    :func:`run_leased_task` outcome."""
...
# ---------------------------------------------------------------------------
# drain with no target task
# ---------------------------------------------------------------------------


def test_resume_empty_dispatcher_drains_nothing() -> None:
    """An empty stack drains to all-zero counters."""
...
def _seed_pending_task_in_sqlite(db_path: str, goal: str = "drain me") -> str:
    """Create and enqueue a task without leasing it; returns its task_id."""
    bundle = build_runtime(
        provider=_ScriptedProvider([]),
        model="test-model",
        system_prompt=SYSTEM_PROMPT,
...
def test_resume_drains_pending_task_via_sqlite(tmp_path) -> None:
    """A pending Task left in the dispatcher is drained (lease → fold →
    run_one_step → release); the provider is reached because it is not
    suspended."""
...
def test_resume_wakes_suspended_task_when_lease_carries_wake_event(
    ...
    """Wake-resume happy path, plus the ``TaskWoken.wake_event`` round trip.

    A suspended parent is woken with a result-populated ``SubtaskCompleted``;
    matching projects on ``subtask_id`` only, so the result rides along purely
    as payload. The drain leases the parent with ``Lease.wake_event`` set and
    threads it into ``engine.note_woken`` before ``run_one_step``, and the
    durable envelope must carry that exact payload back into the EventLog.
    """
...
# ---------------------------------------------------------------------------
# runtime assembly drives a task to terminal
# ---------------------------------------------------------------------------


def test_run_creates_task_with_fake_provider() -> None:
    """A fake provider drives a task built by :func:`build_runtime` all the way
