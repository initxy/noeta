"""The session catalogue read model and the read-only store it projects from.

``SqliteReadOnlyStore`` routinely points at a database another process owns, so
opening a missing file, a foreign sqlite file, or an older schema has to fail
loudly and leave the file byte-identical — a silent create or migrate here
corrupts someone else's store. On top of it, ``list_code_sessions`` and
``filter_code_sessions`` project and filter without writing, and status text is
derived from the typed ``wake_on`` rather than by matching handle strings, so a
new wake kind cannot be mislabelled as resumable.
"""
...
def test_list_filters_to_code_agents_and_skips_malformed() -> None:
    ...
    # non-code session (agent is not one of the official specs)
    log.emit(
        task_id="generic1",
        type="TaskCreated",
        payload=TaskCreatedPayload(
...
# projection over a real on-disk store.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# filter_code_sessions (pure, deterministic; no store)
# ---------------------------------------------------------------------------
...
def test_filter_status_substring_case_insensitive() -> None:
    ...
    # Substring, case-insensitive: 'approval' hits 'awaiting approval'.
...
# ---------------------------------------------------------------------------
# read-path: filter wiring over a real store, zero-write
# ---------------------------------------------------------------------------
