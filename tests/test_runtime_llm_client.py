"""The recording contract RuntimeLLMClient wraps around every model call.

One ``complete(req, ctx)`` must leave exactly one
Started/Recorded/Finished trio on the EventLog with both bodies in the
ContentStore: that ledger is the only durable record of what the model
saw, so even a provider that raises has to produce it instead of letting
the exception escape. The other half is error bucketing — transient /
overflow / fatal, plus the transient backoff loop — because a Policy can
only decide whether to retry from the category stamped on the response.
"""
...
def test_normal_client_emits_three_events_on_success() -> None:
    ...
    # One call_id ties the trio together; without it a concurrent second
    # call would be indistinguishable when reading the stream back.
...
def test_normal_client_emits_three_events_on_provider_exception() -> None:
    ...
    assert resp.content == []
    # A bare (untranslated) provider exception is bucketed as fatal so a
    # Policy can branch on raw['category']; the original message survives
    # under 'error'.
    assert resp.raw == {
        "error": "kaboom",
        "category": "fatal",
...
# ---------------------------------------------------------------------------
# Transient backoff + category buckets
# ---------------------------------------------------------------------------
#
# The backoff loop sits *around the provider call* inside ``complete``, so one
# logical request still records exactly one trio however many attempts it
# takes. A failed attempt writes no request/response pair — only an
# observational ``LLMRetryScheduled`` marker, which fold ignores — so a live
# consumer can see the stall without the ledger growing a second trio.
...
def test_malformed_tool_arguments_budget_exhausted_is_transient_not_fatal() -> None:
    ...
    ``category='transient'`` (NOT 'fatal'): bucketed fatal, one truncated
    tool call would kill the task with zero retries."""
    from noeta.protocols.errors import MalformedToolArgumentsError
    from noeta.runtime.llm import RuntimeLLMClient
...
# ---------------------------------------------------------------------------
# OpenAICompatProvider end-to-end (respx HTTP mock)
# ---------------------------------------------------------------------------
